...
bsl_statement_iterate bsl_statement_iterate_create(bsl_tkn_ir **token, bsl_context *context, bsl_interpreted_code interp, uint32_t *index)
{
	...
	if (curr->token->code != BSLTokenCode_id_generic) {
		...
		char *name = (char *)calloc(1 + curr->token->offset.length, sizeof(char));
	...
	if (curr->token->code != BSLTokenCode_id_generic) {
		...
		local_variable->name = (char *)calloc(1 + curr->token->offset.length, sizeof(char));
	...
	if (curr->token->code == BSLTokenCode_ctl_lbrace) {
		...
		iterate.code.expression = (bsl_expression *)calloc(1, sizeof(bsl_expression));
		...
		while (brace_scope != 0) {
			...
			if (expression->tokens->token != NULL) {
				...
				iterate.code.expression = (bsl_expression *)realloc(iterate.code.expression, sizeof(bsl_expression) * (iterate.code.expression_count + 1));
...
void bsl_statement_iterate_action(bsl_context **context, bsl_statement *statement, bsl_script_offset offset)
{
	...
	bsl_operation *op = (bsl_operation *)calloc(1, sizeof(bsl_operation));
	op->action = bsl_operation_action_cmp_ge;

	op->left_side = (bsl_operation_statement *)calloc(1, sizeof(bsl_operation_statement));
	...
	op->right_side = (bsl_operation_statement *)calloc(1, sizeof(bsl_operation_statement));
